...
public class DynamicBayesNet extends BayesNet implements DynamicBayesianNetwork {
	...
	//Nazli start
	private Set<RandomVariable> X_11 = new LinkedHashSet<RandomVariable>();
	//Nazli end
	...
	public DynamicBayesNet(BayesianNetwork priorNetwork,
						   Map<RandomVariable, RandomVariable> X_0_to_X_1,
						   Map<RandomVariable, RandomVariable> X_1_to_X_1,
						   Set<RandomVariable> E_1, Node... rootNodes) {
		super(rootNodes);
		for (Map.Entry<RandomVariable, RandomVariable> x0_x1 : X_0_to_X_1
				.entrySet()) {
		...
		for (Map.Entry<RandomVariable, RandomVariable> x1_x1 : X_1_to_X_1
				.entrySet()) {
			RandomVariable x1k = x1_x1.getKey();
			RandomVariable x1v = x1_x1.getValue();
			if(!this.X_11.contains(x1v)) {
				this.X_11.add(x1v);
			}
			this.X_1_to_X_1.put(x1k, x1v);
		}
		this.E_1.addAll(E_1);

		// Assert the X_0, X_1, X_00 and E_1 sets are of expected sizes
		Set<RandomVariable> combined = new LinkedHashSet<RandomVariable>();
		combined.addAll(X_0);
		combined.addAll(X_1);
		combined.addAll(X_11);
		combined.addAll(E_1);
	...
	@Override
	public Set<RandomVariable> getX_11() {
		return X_11;
	}
	//Nazli end
